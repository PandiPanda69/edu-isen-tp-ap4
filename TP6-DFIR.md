...
Vous travaillerez en conservant les groupes de travail précédent.
...
Les fichiers `capture.pcap.gpg` et `annexes.zip.gpg` sont chiffrées avec GPG. Votre encadrant vous communiquera le mot de passe pour déchiffrer les ressources au début du TP afin que vous puissiez tous commencer le TP dans les mêmes conditions.

Vous ne savez plus comment déchiffrer avec GPG ? Référez vous au [TP3](https://github.com/PandiPanda69/edu-isen-tp-ap4/blob/main/TP3-crypto.md).
...
3. Jetez un oeil aux paquets de 45 à 81. De quoi s'agit-il ? Pouvez-vous extraire une information importante de ces différents paquets ? Si oui, laquelle ?
...
Une archive au format zip se trouve [ici](https://github.com/PandiPanda69/edu-isen-tp-ap4/raw/main/annexes.zip.gpg). Elle contient les différentes configurations `iptables` des machines du réseau, et un fichier retranscrivant un court interview de l'administrateur système de l'entreprise "Target" avec votre collègue architecte sécurité.
...
| Tarif Journalier Moyen Pentesteur | 1000€ |
